...
async function loadOverviewStats(): Promise<void> {
    try {
        ...
        // Insurance count
        const insuranceCount = (categories as Record<string, number>)['Versicherung'] ||
                              (categories as Record<string, number>)['Versicherungen'] || 0;
 ...
 * Load year expenses
 */
async function loadYearExpenses(year: number): Promise<void> {
    try {
        const data = await api.stats.monthly(year);

        let total = 0;
        const monthlyData = (data as any).total_by_month || {};
        if (Object.keys(monthlyData).length > 0) {
            total = Object.values(monthlyData).reduce((a: number, b: any) => a + (b as number), 0) as number;
        }
 ...
 */
function populateYearSelectors(): void {
    const currentYear = new Date().getFullYear();
    const years: number[] = [];
    ...
    const expensesYearSelect = document.getElementById('expensesYear') as HTMLSelectElement;
    ...
    const compareYear1 = document.getElementById('compareYear1') as HTMLSelectElement;
    const compareYear2 = document.getElementById('compareYear2') as HTMLSelectElement;
 ...
 */
async function updateExpensesPieChart(year: number): Promise<void> {
    try {
        const data = await api.stats.monthly(year);
        const categoryTotals: Record<string, number> = {};

        const categoriesByMonth = (data as any).categories_by_month || {};
        Object.values(categoriesByMonth).forEach((monthCats: any) => {
            Object.entries(monthCats).forEach(([cat, amount]: [string, any]) => {
        ...
        // Create new chart
        const ctx = document.getElementById('expensesPieChart') as HTMLCanvasElement | null;
        if (!ctx) return;
 ...
 */
async function updateYearComparison(): Promise<void> {
    const year1Select = document.getElementById('compareYear1') as HTMLSelectElement | null;
    const year2Select = document.getElementById('compareYear2') as HTMLSelectElement | null;
    ...
    try {
        ...
        const comparison = (data as any).comparison || {};

        const categories = Object.keys(comparison);
        const year1Data = categories.map((cat: string) => (comparison as Record<string, any>)[cat]?.year1 || 0);
        const year2Data = categories.map((cat: string) => (comparison as Record<string, any>)[cat]?.year2 || 0);
        ...
        // Create new chart
        const ctx = document.getElementById('yearComparisonChart') as HTMLCanvasElement | null;
        if (!ctx) return;
 ...
 * Edit insurance
 */
export function editInsurance(insuranceId: string): void {
