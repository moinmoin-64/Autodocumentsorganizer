 ...
 */
async function cacheFirstWithUpdate(
    request: Request,
    cacheName: string
): Promise<Response> {
    const cache = await caches.open(cacheName);
...
self.addEventListener('fetch', (event: any) => {
    ...
    if (url.pathname.startsWith('/api/')) {
        event.respondWith(
            networkFirst(request, CACHE_NAMES.api).catch(() =>
                new Response(
                    JSON.stringify({
                        success: false,
    ...
    if (url.pathname.startsWith('/api/documents/') ||
        ...
        event.respondWith(
            cacheFirst(request).catch(() =>
                new Response('Document not available offline', { status: 503 })
            )
        );
        return;
    }
    ...
    if (request.headers.get('accept')?.includes('image')) {
        event.respondWith(
            cacheFirstWithUpdate(request, CACHE_NAMES.images).catch(() =>
    ...
    event.respondWith(
        networkFirst(request, CACHE_NAMES.assets).catch(() =>
